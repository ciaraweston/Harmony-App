...
const displayItems = () => {
  ...
  itemInfos.forEach(function (el) {
    ...
    const donator = document.createElement("div");
    const id = document.createElement("div");
    const date = document.createElement("div");

    item.textContent = el.item;
    quantity.textContent = el.quantity;
    donator.textContent = el.donator;
...
const saveItems = function (e) {
  ...
  let donator = document.querySelector("#input3").value;
  let date = new Date();
  let completed = false;
...
const editItem = e => {
  ...
  let donator = document.querySelector("#input3");
  ...
  for (let i = 0; i < itemInfos.length; i++) {
    if (itemInfos[i].id2 == e.target.id) {
      ...
      donator.value += itemInfos[i].donator;
      itemInfos.splice(i, 1);
